...
- **Says which machine it measured**: `/vitals` reports `source.scope` as `host` or `container`, with the procfs and disk paths used. Host figures require `PROCFS_PATH=/host/proc` and the host root mount, both set by the bundled compose file; without them the numbers describe the container and say so.
